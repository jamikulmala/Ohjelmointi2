...
void GameBoard::delete_random_element()
{
     // counter to prevent program from crashing, if deletable
     // elements cannot be found.
     int not_found = 0;
     NumberTile* to_delete = nullptr;
    // finding a random tile that is not empty or a big number
    while(true){
    int random1 = std::rand() % SIZE;
    int random2 = std::rand() % SIZE;
    std::vector <NumberTile*> select_vector = board_[random1];
    NumberTile* select_element = select_vector[random2];
    // The bigger number tiles wont get deleted, that would
    // only make the game harder not easier
    if(select_element->value() <= 8 && select_element->value() != 0){
        to_delete = select_element;
        break;
    }
    else if(not_found == 16){
        break;
        return;
    }
    else{
