...
bool checknumber(std::string number){
    for(char& piece : number){
        if(NUMBERS.find(piece) == std::string::npos){
            return false;
        }
}
    return true;
...
void update_file(std::string file_name,std::map <std::string,Student *> user_ids){
...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            student_numbers.at(parts.at(1))->phone_number = number;
